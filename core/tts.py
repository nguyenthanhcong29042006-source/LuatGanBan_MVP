# -*- coding: utf-8 -*-
"""NHIỆM VỤ 1b — Phát giọng nói đa ngôn ngữ (Đảm bảo 100% luôn có âm thanh dự phòng)."""
...
def tts_tieng_viet(text: str, *, tag: str = "vi") -> Path | None:
    """Tạo audio MP3 tiếng Việt siêu nén (luôn sẵn sàng làm dự phòng)."""
...
def phat_tieng_mong(
    ...
    *,
    ...
    """Điều phối phát âm thanh tiếng Mông. 
    Đảm bảo nếu không tìm thấy file thu sẵn hoặc local neural, 
    sẽ tự động chuyển sang phiên âm tiếng Việt để luôn có âm thanh phát ra cho bà con.
    """
    provider = provider or TTS_HMONG_PROVIDER
    if provider == "off" or not (text_rpa or "").strip():
        return None, "off"

    # Ép danh sách tầng luôn có vi_phonetic ở cuối làm phao cứu sinh an toàn tuyệt đối
    thu_tu = ["audio_bank", "local_neural", "vi_phonetic"]
    ...
    for tang in thu_tu:
        ...
        elif tang == "vi_phonetic":
            p = tts_vi_phonetic(text_rpa)

        if p is not None and Path(p).exists():
            return p, tang
            
    # Dự phòng cuối cùng: đọc trực tiếp văn bản thô qua gTTS
    p_cuoi = tts_tieng_viet(text_rpa, tag="fallback_raw")
    if p_cuoi and Path(p_cuoi).exists():
        return p_cuoi, "vi_phonetic"
...
NHAN_TANG = {
    ...
    "vi_phonetic": "🔤 Giọng đọc hướng dẫn hỗ trợ",
    "off": "🔇 Đã tắt giọng",
    "that_bai": "⚠️ Đang cập nhật âm thanh",
}
